...
docker run -d --rm --name=sensu-server-metrics --link sensu-api --link redis --link rabbitmq --link influxdb -v $DIR/volumes/sensu/logs:/var/log/sensu terjesannum/sensu-server-metrics:2
docker run -d --rm --name=sensu-server-events-s --link sensu-api --link redis --link rabbitmq --link influxdb -v $DIR/volumes/sensu/logs:/var/log/sensu terjesannum/sensu-server-events-s:2
docker run -d --rm --name=sensu-server-events-n --link sensu-api --link redis --link rabbitmq --link influxdb -v $DIR/volumes/sensu/logs:/var/log/sensu terjesannum/sensu-server-events-n:2
